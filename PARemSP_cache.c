...
int main()
{
	int row=0,chart=0,column=0,act_col;
...
//	fp3 = fopen("Comp.txt","a+");
	...
	row = row + 2;
	column = column +2;
	act_col = column/16; // as cache line size is 64 so it can take 64/4 = 16 int elements in one line.
	if((column%16) != 0)
		act_col++;
	chart = row * column;
	...
	for(i = 0; i < row; i++)
	{
		...
		label[i] =(int *) malloc(act_col*16 * sizeof(int));
